...
class InsiderGaming:
    ...
    def get_write_body(cls):
        ...
        headers = InsiderGaming.headers
        headers['Referer'] = 'https://insider-gaming.com/sitemap_index.xml'
        
        try: response = requests.get(InsiderGaming.url, headers=headers)
        except Exception as e:
            print(C.RED(f'\n[!InG]Aborted: {e}'))
            return False
        response.encoding = 'utf-8'
        InsiderGaming.body_html_path.write_text(response.text)
    ...
    def get_article_url(cls):
        ...
        body_html = InsiderGaming.body_html_path.read_text(encoding='utf-8')
        body = BeautifulSoup(body_html, 'xml')
        
        #write into local 
        f = open(str(InsiderGaming.article_url_path), 'w', encoding='utf-8')
        print(C.YELLOW('[InG]Writing article url...'), end='', flush=True)
        
        url_tags = body.find_all('url')
        for tag in url_tags[:30]:
    ...
    def cls_output_article(cls, title_url_dict: dict[str, str]):
        ...
        response = requests.get(url, headers=InsiderGaming.headers)
        ...
        #write in article
        output_path = InsiderGaming.output_path / pub_date / f'{title_prefix}….txt'
        try:
        ...
        try:
            ...
            thumbnail_url = re.search(r'(https:.*\.(?:png|jpg))', str(thumbnail_url))#type: ignore
        ...
        for i, img_url in enumerate(img_url_list):
            try:
                image_response = requests.get(img_url, headers=InsiderGaming.headers, stream=True)#type: ignore
...
if __name__ == '__main__':
    d1 = {"When is The Hollow Knight: Silksong Review Embargo Date?": "https://insider-gaming.com/when-is-the-hollow-knight-silksong-review-embargo-date/"}
    a = InsiderGaming.cls_output_article(d1)
    print(len(a))
    pass
