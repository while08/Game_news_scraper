...
class GamesRadar:
    ...
    def get_write_body(cls):
        ...
        response = requests.get(GamesRadar.url, headers=GamesRadar.headers)
        soup = BeautifulSoup(response.text, 'lxml').article
        body = soup.find('div', class_='listingResults')#type: ignore
        GamesRadar.body_html_path.write_text(body.prettify())#type: ignore
    ...
    def get_article_url(cls):
        ...
        body_html = GamesRadar.body_html_path.read_text(encoding='utf-8')
        soup = BeautifulSoup(body_html, 'lxml').div
        
        print(C.YELLOW('[GR]Writing article url...'), end='', flush=True)
        
        f = open(str(GamesRadar.article_url_path), 'w', encoding='utf-8')
    ...
    def cls_output_article(cls, title_url_dict: dict[str, str]):
        ...
        response = requests.get(url, headers=GamesRadar.headers)
        ...
        #write in article
        output_path =GamesRadar.output_path / pub_date / f'{title_prefix}….txt'
        try:
            output_path.parent.mkdir(parents=True, exist_ok=True)
            output_path.write_text(article, encoding='utf-8')
            print(C.YELLOW('Complete, getting images...'))
        except Exception as e:
            print(C.RED(f'\n[!GR]Raise error while writing article: {e}'))
            return False
        ...
        for tag in target_div.find_all('figure', recursive=False):#type: ignore
            try:
                ...
                print(C.RED(f'[!GR]Image error:{e}'))
        
        #download images
        for i, img_url in enumerate(img_url_list):
            try:
                image_response = requests.get(img_url, headers=GamesRadar.headers, stream=True)#type: ignore
                ...
                print(C.RED(f'[!GR]Failur to download {i+1}image: {e}'))
                continue
...
if __name__ == '__main__':
    ...
    GamesRadar.cls_output_article(d)
    pass
